...
PROG_NAME="Photos Backup to Dropbox (TEST)"
...
rsync -a --delete --dry-run --verbose \
...
STATUS=$?
if [ $STATUS -eq 0 ]; then
    echo "$(date): Dry-run complete." >> "$LOG"
    notify "$PROG_NAME dry-run completed successfully."
else
    echo "$(date): Dry-run failed (exit $STATUS)." >> "$LOG"
    notify "$PROG_NAME dry-run failed. Check ~/Library/Logs/photos-backup-test.log for details."
fi
